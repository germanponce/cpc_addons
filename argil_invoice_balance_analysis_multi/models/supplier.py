...
SELECT l.id as id, l.partner_id as partner_id, res_partner.name as "partner_name",
    ...
    def load_views(self, views, options=None):
        res=super(account_invoice_supplier_collection_projection, self).load_views(views, options)
        dlp = self.env['ir.values'].get_default('account.config.settings', 'days_limit_projection')
...
class account_invoice_supplier_balance_due(models.Model):
    _inherit = 'account.invoice.supplier_balance_due'

    argil_sql_str_for_overdue = """
                create or replace view account_invoice_supplier_balance_due as (
SELECT l.id as id, l.partner_id as partner_id, res_partner.name as "partner_name",
              ai.type invoice_type, ai.user_id,
    ...
    @api.model
    def load_views(self, views, options=None):
        res=super(account_invoice_supplier_balance_due, self).load_views(views, options)
        dlp = self.env['ir.values'].get_default('account.config.settings', 'days_limit_projection')
        res['fields_views']['list']['fields']['days_due_01to30']['string']="1 - %s" % (dlp)
        res['fields_views']['list']['fields']['days_due_31to60']['string']="%s - %s" % (dlp+1, dlp*2)
        res['fields_views']['list']['fields']['days_due_61to90']['string']="%s - %s" % (dlp*2+1, dlp*3)
